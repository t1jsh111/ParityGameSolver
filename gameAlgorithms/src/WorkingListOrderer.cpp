...
WorkingListOrderer::getInitialWorkingList(const std::vector<std::shared_ptr<Node>> &nodes) {
...
void WorkingListOrderer::appendPredecessorsOfNodeToWorkingList(const Node& node, std::list<std::shared_ptr<Node>> &workingList, const std::unordered_map<int, ProgressMeasure>& rhoMapping) {
